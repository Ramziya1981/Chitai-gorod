...
def test_add_product_to_cart():
    ...
    with allure.step("Добавить книгу в корзину"):
        product_id = 3078656  # ID продукта для добавления
...
def test_delete_product_from_cart():
    ...
    """
    product_id = 3078656  # ID добавленной книги
    item_list_name = "search"  # Имя списка, откуда добавляется продукт
    ...
    with allure.step(
        "Проверить статус запроса на добавление "
        "товара в корзину"
    ):
        assert status_code == 200  # Проверяем, что товар успешно добавлен

    # Получаем содержимое корзины, чтобы убедиться, что
    # добавленный товар есть в ней
    ...
    with allure.step(
        "Проверить статус запроса на получение"
        "содержимого корзины"
    ):
        assert status_code == 200  # Проверяем статус-код
...
@allure.story(
    ...
    "используя неправильный метод (PATCH вместо POST)"
)
def test_wrong_add_request():
    ...
    with allure.step("Попытка добавить книгу в корзину некорректно"):
        product_id = 3078656  # ID продукта для попытки добавления
...
def test_add_product_to_cart_with_empty_body():
    ...
    with allure.step("Проверить статус запроса"):
        ...
        assert status_code == 422, (
            ...
            "но получен статус {}".format(status_code)
        )
